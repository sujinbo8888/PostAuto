...
public class MainActivity extends BaseSettingsActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button startOther=(Button)findViewById(R.id.startWY);
        startOther.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent=getPackageManager().getLaunchIntentForPackage("com.netease.newsreader.activity");
                startActivity(intent);
            }
        });

        Button BtnSocket=(Button)findViewById(R.id.socket);
        BtnSocket.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //SocketService.startActionSocketClient(MainActivity.this,"192.168.1.2",5000,"nihao 你好");
        ...
        intentFilter.addAction(SocketService.BROADCAST_RECEIVE_MSG);

        registerReceiver(intentSocketReceiver, intentFilter);
    }
    ...
    private BroadcastReceiver intentSocketReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String msg=intent.getStringExtra(SocketService.RECEIVE_MSG_TAG);

            DataScheduler.newInstance().init(msg);

            Toast.makeText(getBaseContext(), "socket receive!"+msg,
